...
y_U280 = np.array([3338, 5872, 7460, 11035, 8276, 4198, 2130])
y_U250 = np.array([1205, 2223, 3190, 5532, 7045, 4048, 533])
...
speedup_array = y_U250 / y_U280
max_speedup = np.amax(speedup_array)
min_speedup = np.amin(speedup_array)
print("Speed U250/U280:\n{}\nmax: {:.2f}x\nmin: {:.2f}x".format(speedup_array, max_speedup, min_speedup))

print("Best performance:\nU280:{}\nU250:{}\nratio:{}".format(np.amax(y_U280), np.amax(y_U250), np.amax(y_U280) / np.amax(y_U250)))

fig, ax = plt.subplots(1, 1, figsize=(8, 2))
# 
rects1  = ax.bar(x - width / 2, y_U280, width)#, label='Men')
rects2   = ax.bar(x + width / 2, y_U250, width)#, label='Women')
...
legend_list = ['U280 FPGA (with HBM)', 'U250 FPGA (no HBM)']
...
ax.set(ylim=[0, np.amax(y_U280) * 1.5])

plt.rcParams.update({'figure.autolayout': True})

plt.savefig('./out_img/experiment_6_FPGA_model.png', transparent=False, dpi=200, bbox_inches="tight")
plt.show()
