# Example usaage: 
#   python CPU_GPU_FPGA_max_throughput_comparison.py --legend_loc_x 0.5
...
parser.add_argument('--legend_loc_x', type=float, default=0.4, help="the x position of legend, 0~1")
...
y_cpu = [max_qps_cpu_R1, max_qps_cpu_R10, max_qps_cpu_R100]
y_fpga = [max_qps_fpga_R1, max_qps_fpga_R10, max_qps_fpga_R100]
y_fpga_optimized = [max_qps_fpga_optimized_R1, max_qps_fpga_optimized_R10, max_qps_fpga_optimized_R100]
y_gpu = [max_qps_gpu_R1, max_qps_gpu_R10, max_qps_gpu_R100]
...
rects_fpga_optimized = ax.bar(x + 0.5 * width, y_fpga_optimized, width)#, label='Women')
rects_gpu  = ax.bar(x + 1.5 * width, y_gpu, width)#, label='Men')
...
best_qps = np.amax(y_gpu)
ax.set(ylim=[0, best_qps * 1.25])

plt.rcParams.update({'figure.autolayout': True})

plt.savefig('./CPU_GPU_FPGA_throughput_comparison_fig/max_CPU_GPU_FPGA.png', transparent=False, dpi=200, bbox_inches="tight")
plt.show()
