...
dbname_list = ['SIFT100M', 'SIFT100M', 'SIFT100M', 'Deep100M', 'Deep100M', 'Deep100M']
topK_list = [1, 10, 100, 1, 10, 100]
recall_goal_list = [0.3, 0.8, 0.95, 0.3, 0.7, 0.95]
...
cpu_performance_dict_list = [
    ...
    'cpu_response_time_Deep100M_qbs_1_m5.4xlarge.pkl', 'cpu_response_time_Deep100M_qbs_1_m5.4xlarge.pkl', 'cpu_response_time_Deep100M_qbs_1_m5.4xlarge.pkl']
...
fpga_performance_dict_list = [
    ...
    'FPGA_perf_dict_Deep100M_K_1.pkl', 'FPGA_perf_dict_Deep100M_K_10.pkl', 'FPGA_perf_dict_Deep100M_K_100.pkl']
...
assert len(dbname_list) == len(topK_list) and \
    ...
    len(dbname_list) == len(fpga_performance_dict_list) 
...
# 
rects_cpu  = ax.bar(x - width, cpu_best_latency_list, width)#, label='Men')
rects_fpga = ax.bar(x , fpga_best_latency_list, width)#, label='Women')
rects_autogen = ax.bar(x + width, autogen_best_latency_list, width)
...
ax.legend([rects_autogen, rects_cpu, rects_fpga], legend_list, facecolor='white', framealpha=1, frameon=False, loc='upper center', fontsize=legend_font, ncol=3)
...
plt.savefig('./CPU_FPGA_throughput_comparison_fig/CPU_FPGA_latency_comparison.png', transparent=False, dpi=200, bbox_inches="tight")
plt.show()
